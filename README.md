...
1. Bump `package.json`, update `CHANGELOG.md`, then merge the change.
2. Publish a GitHub release with the tag `<version>`; the workflow attaches the package files automatically. Alternatively, run **Release VPM package** manually from `main` or a version tag. Use `dry_run` to build and verify without publishing.
3. `VPM_REPOS_TOKEN` can trigger the VPM listing rebuild. Otherwise, run **Build Repo Listing** in the `TenteEEEE/vpm-repos` Actions tab.
